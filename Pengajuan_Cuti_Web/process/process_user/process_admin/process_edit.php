...
if (empty($nama_admin) ||   
    ...
    empty($pass_user)||
    empty($unit_kerja)) {
    header("location: " . BASE_URL . 'dashboard_admin.php?page=edit_user&process_user=failed');
} else {
    ...
    $query_user = mysqli_query($koneksi, "UPDATE tbl_multi_user SET 
    ...
    if ($query_jabatan && $query_golongan && $query_user && $query_pegawai ) {
        header("location: " . BASE_URL . 'dashboard_admin.php?page=home_user&status=success');
    } else {
        header("location: " . BASE_URL . 'dashboard_admin.php?page=edit_user&process_user=failed');
